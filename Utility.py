...
IMAGES = {
    ...
    "right_arrow": (
        "Resources/images/right_arrow_white.png",
        "Resources/images/right_arrow_white.png",
    ),
}
ANIM_FOLD_PATH = {
    "AI": (
        "Resources/images/bot dark",
        "Resources/images/bot dark",
    ),  # (<light>, <dark>)
    "friends": (
        "Resources/images/friends dark",
        "Resources/images/friends dark",
    ),  # (<light>, <dark>)
...
BG_VID_PATH = (
    "Resources/light bg.mp4",
    "Resources/dark bg.mp4",
)  # taking too long to load so not in use
...
COLOR = {
    ...
    "font": {"1": ("#BCBCCC", "#BCBCCC"), "2": ("#343145", "#BCBCCC")},
    "button": {"normal": ("#4f4a68", "#343145"), "hover": ("#343145", "#4f4a68")},
    "switch": {
...
class DataStorePath:
    ...
    def get_appdata():
        ...
        print(
            "File stored in", path
        )  # displaying it for developer to know where it is stored
        if not os.path.exists(path):  # create intelli chart main folder in appdata
            os.mkdir(path)
        res_path = os.path.join(
            appdata_path, "Intelli Chat", "Resources"
        )  # Generate corresponding path
        if not os.path.exists(res_path):  # create Resource sub folder in intelli chart
            os.mkdir(res_path)
        p_pic_path = os.path.join(
            appdata_path, "Intelli Chat", "Resources", "profile_pic"
        )  # Generate corresponding path
        if not os.path.exists(p_pic_path):  # create profile pic sub folder in Resource
            os.mkdir(p_pic_path)
        memory_path = os.path.join(
            appdata_path, "Intelli Chat", "Resources", "Memory"
        )  # Generate corresponding path
        if not os.path.exists(memory_path):  # create Memory sub folder in Resource
            os.mkdir(memory_path)
        images_path = os.path.join(
            appdata_path, "Intelli Chat", "Resources", "images"
        )  # Generate corresponding path
        if not os.path.exists(
            images_path
        ):  # create images sub folder in Resource to store user profile pic
            os.mkdir(images_path)
        rec_images_path = os.path.join(
            appdata_path, "Intelli Chat", "Resources", "rec_images"
        )  # Generate corresponding path
        if not os.path.exists(
            rec_images_path
        ):  # create rec_images sub folder in Resource to store incoming images
...
class LogCollect(ctk.CTkCheckBox):
    ...
    def run():
        images = {
            "logo ICO": "Resources/images/logo.ico",
        ...
        LogCollect.log_disp = CTkListbox.CTkListbox(
            frame,
            border_color="#201c1c",
            fg_color="#000000",
            text_color="white",
        ...
        for j, i in enumerate(collector):
            i = map(str, i)
            i = " ".join(i)
            if LogCollect.log_disp.size() > 0:
                for k in range(LogCollect.log_disp.size()):
                    if j == int(LogCollect.log_disp.get(k)[0][:-1]):
                        break
                else:
                    LogCollect.log_disp.insert(
                        LogCollect.log_disp.size() - 1, [f"{j}.", i]
                    )
                    LogCollect.log_disp.update()
            else:
                LogCollect.log_disp.insert(LogCollect.log_disp.size() - 1, [f"{j}.", i])
                LogCollect.log_disp.update()
...
class AnimatedButton(ctk.CTkButton):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(master, image=self.frames[self.frame_index], **kwargs)
        ...
        for path in (light, dark):
            ...
            for _, __, img_data in os.walk(path):
                sorted_data = sorted(
                    ...
                    key=lambda x: int(
                        x.split(".")[0][-4:]
                    ),  # key performs any function on the data before applying sort
                )
...
class Video(TkinterVideo):
    ...
    def bg_play(self):
        ...
        if self.audio_path:  # Play audio if externally provided
            threading.Thread(
                target=SoundManager.play, args=(self.audio_path,), daemon=True
            ).start()
