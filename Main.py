...
class Main:
    def __init__(
        ...
        password: str,
        profile_address: str = IMAGES["user"],
    ...
    def run(self, app: ctk.CTk):
        ...
        label_logo = ctk.CTkLabel(
            master=frame, text="", image=logo_img, anchor="center"
        )
        ...
        def friends():
            ...
            if self.username == "":
                ...
                try:
                    ...
                    lobby_public_partner = rsa.PublicKey.load_pkcs1(
        ...
        class SettingPanel(ctk.CTkFrame):
            def __init__(
                ...
                profile_pic = ctk.CTkImage(
                    Utility.Images.open_as_circle(profile_pic_address), size=(250, 250)
                )
                ...
                appearance_mode_switch.place(
                    relx=0.05, rely=0.99, anchor="sw", relwidth=1
                )

                # slide back button
                slide_back_image = ctk.CTkImage(
                    *map(Image.open, IMAGES["right_arrow"]), size=(20, 20)
                )
            ...
            def logout(self, main_class_object: Main):
                ...
                profile_pic = ctk.CTkImage(
                    Utility.Images.open_as_circle(IMAGES["def_profile"]),
                    size=(250, 250),
                )
            ...
            def display_elements_on_cond(self, main_class_object: Main):
                ...
                if main_class_object.username:  # settings panel when logged in
                    ...
                    #  display username
                    self.u_name_label.configure(
                        text=f"Welcome! {main_class_object.username}"
                    )
                ...
                if main_class_object.username:  # Checks if the user has logged in
                    if main_class_object.profile_address == IMAGES["user"]:
                        ...
                        if file_path:
                            ...
                            main_class_object.profile_address = (
                                os.path.join(STORE_PATH, IMAGES_FOLD_PATH)
                                + file_path.split("/")[-1]
                            )

                            abs_target_dir = (
                                STORE_PATH + "\\" + IMAGES_FOLD_PATH.replace("/", "\\")
                            )
            ...
            def animate_forward(self):
                ...
                if self.pos > self.end_pos:  # frame has not reached the destination
                    ...
                    self.place(
                        relx=self.pos, rely=0.005, relwidth=self.width, relheight=0.985
                    )
    ...
    def login(self, bg_l1: ctk.CTkLabel, app: ctk.CTk, join=True, setting_panel=None):
        ...
        login_mes = ctk.CTkLabel(
            master=frame,
            width=100,
            height=50,
            text="Kindly enter your ID and password",
            font=FONT["error"],
            anchor="center",
        )
        ...
        def enter():
            ...
            if (
                entry.get() != "" and entry_pass.get() != ""
            ):  # check if user provided the credentials
                try:  # connect to server and validate credentials
                    ...
                    out = eval(
                        rsa.decrypt(self.client_socket.recv(1024), private_key).decode(
                            "utf-8"
                        )
                    )
                    time.sleep(0.05)
    ...
    def sign_up(self, bg_l1: ctk.CTkLabel, app: ctk.CTk):
        ...
        def add_user():
            ...
            if (
                entry.get() != ""
                and entry_pass.get() != ""
                and entry_pass_confirm.get() != ""
            ):  # check if fields are filled
                if (
                    entry_pass.get() == entry_pass_confirm.get()
                    ...
                    try:  # connect to the server and request it to add the user to the database
                        ...
                        out = eval(
                            rsa.decrypt(
                                self.client_socket.recv(1024), private_key
                            ).decode("utf-8")
                        )
                        time.sleep(0.05)
                        print(out)
                        ...
                        Utility.Message.display(
                            "Couldn't connect to server, please try again", 2
                        )
                else:  # Tell user to confirm password
                    Utility.Message.display(
                        "Password not matching with confirm password", 1
                    )
    ...
    def close_window(self):
        ...
        Main.save_login_cred(
            self.username, self.password, self.profile_address
        )  # Also Closes The log console, if not closed
        root.quit()
        root.destroy()
    ...
    def save_login_cred(username: str, password: str, profile_address: str):
        ...
        encrypted = fernet.encrypt(
            f"{username}:{password}:{profile_address}".encode("utf-8")
        )
...
if __name__ == "__main__":
    ...
    ctk.set_appearance_mode(
        "dark"
    ) if Utility.APPEARANCE_MODE == 1 else ctk.set_appearance_mode("light")
    ...
    intro = Utility.Video(
        root,
        "Resources/Intro2.mp4",
        "Resources/Intro2.wav",
        on_video_end,
        scaled=True,
        keep_aspect=False,
    ...
    root.minsize(
        1200, 712
    )  # the values are adjusted (when tkinter converts it becomes [1500, 890])

    # For Splash screen to close
    if getattr(sys, "frozen", False):
        pyi_splash.close()

    main = Main("", [], "")
    root.protocol(
        "WM_DELETE_WINDOW", main.close_window
    )  # map custom close function to window
    ...
    def resizing_mid():
        ...
        root.geometry(
            f"{SIZE_X}x{SIZE_Y}+{x_coordinate}+{y_coordinate}"
        )  # Set screen size
    ...
    def get_btn_images():
        ...
        Utility.AI_IMAGE_SEQUENCE = Utility.AnimatedButton.import_folders(
            Utility.ANIM_FOLD_PATH["AI"][0], Utility.ANIM_FOLD_PATH["AI"][1], (30, 30)
        )
        Utility.FRIEND_IMAGE_SEQUENCE = Utility.AnimatedButton.import_folders(
            Utility.ANIM_FOLD_PATH["friends"][0],
            Utility.ANIM_FOLD_PATH["friends"][1],
            (30, 30),
        )
        Utility.SETTING_IMAGE_SEQUENCE = Utility.AnimatedButton.import_folders(
            Utility.ANIM_FOLD_PATH["setting"][0],
            Utility.ANIM_FOLD_PATH["setting"][1],
            (30, 30),
        )
